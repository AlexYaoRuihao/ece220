...
//  The statement, given char post[2]; and four integers w,x,y,z,
	...
	{
		int guess_int[]={w,x,y,z};
		int sol_int[]={solution1,solution2,solution3,solution4};

		if (w>0 && w<9 && y>0 && y<9 && x>0 && x<9 && z>0 && z<9)
		{
		for (int i=0; i<4; i++)
		{
				
			if (guess_int[i]==sol_int[i])
			{
				p_match++;
				sol_int[i]=0;
				guess_int[i]=0;
			...
			for(int j=0; j<4; j++)
			{
				...
				if(sol_int[i]==guess_int[j] && sol_int[i]!=0)
				{
					m_match++;
					sol_int[i]=0;
					guess_int[j]=0;
