/*INTROPARAGRPH
in this MP I  worte functions that modify tree node like roate,and node sway and function that change topology of tree . i also worte a 
postfix travasal for get the expression of tree. In this mp,  i really gained more experience on binary tree and solving prob through recursion
*/
...
int is_leaf_node(node_t* ptr) {
  // TODO: (remember to modify the return value appropriately)
	if (ptr->left==NULL && ptr->left==NULL){ //check left & right
		return 1;
	}
  	return 0;
}
...
int is_internal_node(node_t* ptr) {
  // TODO: (remember to modify the return value appropriately)
	if (ptr->module==NULL){//check module
		return 1;
	}
  	return 0;
}
...
int is_in_subtree(node_t* a, node_t* b) {
  // TODO: (remember to modify the return value appropriately)
	...
	if(a->left == b || a->right == b || a==b){
		return 1;
	}
	return is_in_subtree(a->left, b) || is_in_subtree(a->right,b);//using recursion check whole subtree
...
void recut(node_t* ptr) {
  ...
  // TODO: 
  if(ptr->cutline==H){//change cutline
  	ptr->cutline=V;
  	return;
  }
  if (ptr->cutline==V){
...
void swap_module(node_t* a, node_t* b) {
  ...
  // TODO:
  module_t* temp = a->module;//sway with temp
  a->module = b->module;
  b->module = temp;
}
...
void swap_topology(node_t* a, node_t* b) {
  ...
  // TODO:
  node_t* tempar;//swap parent and child pointers
  tempar = a->parent;
  if(tempar->left==a){
  	tempar->left=b;
  }
...
node_t* init_slicing_tree(node_t* par, int n) {
 ...
 node_t* ptr  = (node_t*)malloc(sizeof(node_t));
  ...
  {
  	node_t* ptrR = (node_t*)malloc(sizeof(node_t));
  	ptrR->module = &modules[n];
  	ptrR->cutline = UNDEFINED_CUTLINE;
  	ptrR->parent = ptr;
  	ptrR->left = NULL;
  	ptrR->right = NULL;
  	...
  	ptr->parent = par;
  	ptr->right = ptrR;
  	ptr->left = init_slicing_tree(ptr,n+1);                          //recursivly call the left subtree.
  	return ptr;
}
